"""
train_model_concept_bottleneck_mlp.py — MLP on τ-masked max-pooled CRAFT vector z.

Pairs with ``graph_concept_bottleneck_mlp_linear`` (``CBM_GRAPH_VARIANT=concept_bottleneck_mlp_linear``)
and ``model_concept_bottleneck_mlp``. Selected when ``FRONTEND=cb_mlp`` in ``sbatch_train_gcbm.sh``.
"""
...
from model_concept_bottleneck_mlp import ConceptBottleneckMLP
